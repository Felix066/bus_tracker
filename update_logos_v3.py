...
for filename in os.listdir('.'):
    if filename.endswith(".html"):
        ...
        # Replace old SVG with clipPath id="w-clip"
        new_content = re.sub(r'<svg[^>]*>.*?<clipPath id="w-clip".*?</svg>', new_svg, content, flags=re.DOTALL)
        
        # Replace <i class="fas fa-bus"></i> BusTrack 
        new_content = re.sub(r'<div class="logo">\s*<i class="fas fa-bus"></i>\s*(.*?)\s*</div>', r'<div class="logo">\n      ' + new_svg + r'\n      <span class="brand-text">\1</span>\n    </div>', new_content, flags=re.DOTALL)
        new_content = re.sub(r'<div class="brand">\s*<i class="fas fa-bus"></i>\s*(.*?)\s*</div>', r'<div class="brand">\n      ' + new_svg + r'\n      <span class="brand-text">\1</span>\n    </div>', new_content, flags=re.DOTALL)
