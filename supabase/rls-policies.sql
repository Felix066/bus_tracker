...
DROP POLICY IF EXISTS "Enable read for authenticated users" ON trips;
DROP POLICY IF EXISTS "Enable read for authenticated users" ON bus_locations;
DROP POLICY IF EXISTS "Enable read for authenticated users" ON computed_locations;
DROP POLICY IF EXISTS "Enable read for authenticated users" ON stop_arrivals;
DROP POLICY IF EXISTS "Enable read for authenticated users" ON profiles;
DROP POLICY IF EXISTS "Enable insert for all roles" ON bus_locations;
DROP POLICY IF EXISTS "Enable insert for drivers" ON computed_locations;
DROP POLICY IF EXISTS "Enable insert for drivers" ON stop_arrivals;
DROP POLICY IF EXISTS "Enable insert for individual users" ON profiles;
DROP POLICY IF EXISTS "Enable trip management for drivers" ON trips;
